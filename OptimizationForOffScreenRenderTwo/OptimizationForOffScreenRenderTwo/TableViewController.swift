...
class TableViewController: UITableViewController {
    ...
    let customeCellIdentifier = "CustomCell"
    ...
    override func viewDidLoad() {
        ...
        self.tableView.register(TableViewCell.classForCoder(), forCellReuseIdentifier: customeCellIdentifier);
...
//        let cell = tableView.dequeueReusableCell(withIdentifier: cellIdentifier, for: indexPath) //使用Storyboard
        ...
        if labelL != nil {
            (labelL as! UILabel).text = "OffscreenRender" + String(indexPath.row)
        }
        
        if labelR != nil {
          (labelR as! UILabel).text = String(indexPath.row) + "离屏渲染"
        }
